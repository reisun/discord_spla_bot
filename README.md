...
ただし、メンバーの人数と役職なし（＝村人）の人数に気を付けてください。  
上記の例の場合、メンバーが４人だと村人は０人となります。  
...
* 追加
```
/spj_eject -add @ユーザーのメンション（複数指定可能）
```
* 取り消し
```
/spj_eject -delete @ユーザーのメンション（複数指定可能）
```
* 現在の追放者を確認
```
/spj_eject
...
## あまり使わないコマンド
基本操作は上記のコマンドで可能です。  
以下は、こういうことできないかなーとか、ちょっと動作がおかしいからリセットしたいなーとか  
そういう時に使うやつらなので興味のある人だけ。

#### メンバーの追加・削除
* 追加
```
/spj_member -add @ユーザーのメンション（複数指定可能）
```
* 削除
```
/spj_member -delete @ユーザーのメンション（複数指定可能）
```
* 現在のメンバー確認
```
/spj_member
...
#### オプション：知れる  
    ...
    （コマンド例）
    /spj_send_role_option 狂人>知れる>人狼
